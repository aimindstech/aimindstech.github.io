(function(){const doc=document.documentElement;const parentURL='https://aimindstech.github.io/';function createEl(element='div'){return document.createElement(element);}
...
doc.addEventListener('click',function(event){let target=event.target;const isNavDrop=isTarget(target,'nav-drop',true);const isNavClose=isTarget(target,'nav-close',true);const isNavBar=isTarget(target,'nav_bar-wrap');isNavDrop||isNavClose||isNavBar?toggleMenu():false;});})();(function share(){let share=elem('.share');let open='share-open';let close='share-close';let button=elem('.share-trigger');function showShare(){pushClass(share,open);deleteClass(share,close);}
